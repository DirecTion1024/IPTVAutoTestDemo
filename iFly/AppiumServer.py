...
class AppiumServer(object):
    ...
    def StartServer(self, udid=None, wait=False):
        ...
        cmd = "appium -U " + udid + " -p " + str(port) + " --session-override --log-level error"
    ...
    def GetRunningTasks(self, udid=None):
        ...
        re_process = re.compile(r'''\s*(?P<pid>\d+)\s+(?P<lstart>\w+\s+\w+\s+\d+\s+\d+\:\d+\:\d+\s+\d+)\s+(?P<cmd>.*)''', re.X)
        ret = []
        if not out:
            return []
    ...
    def isDeivceIdle(self, udid=None):
        ...
        if not udid in devicelist:
            raise AssertionError('Device %s not connected! Please plugin the decvice!' % udid)
